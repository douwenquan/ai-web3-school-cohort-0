...
AI Agent 处理链上数据、用户上下文和交易信息时，涉及用户身份、资产和行为的隐私边界。

### 隐私风险

| 风险 | 说明 |
|------|------|
| **链上透明性** | 所有 Agent 操作记录在链上公开可查 |
| **上下文泄露** | Agent 读取的链上数据可能暴露用户策略和持仓 |
| **身份关联** | 多笔交易可能被关联分析出用户画像 |

### 应对方向

| 方向 | 说明 |
|------|------|
| **零知识证明（ZK）** | 验证交易有效性而不披露细节 |
| **隐私 AI Agent** | Agent 在 TEE 或本地执行推理，不上传原始数据 |
| **选择性披露** | 只暴露必要信息，隐藏策略细节和持仓 |

---

## 八、AI Sovereignty（AI 主权）

### 核心问题

谁控制模型的训练、推理和部署？用户的数据归谁？用户能不能脱离平台自治？
...
主权 AI 意味着 Agent 不依赖单一模型供应商、用户数据不离开用户控制范围、Agent 逻辑可审计和修改。去中心化 AI 基础设施（模型市场、算力市场）是实现 AI 主权的基础支撑。

---

## 九、Governance AI（治理 AI）
...
| 10 | AI Privacy | 链上透明性与隐私边界的平衡 |
| 11 | AI Sovereignty | 谁控制模型和数据的主权问题 |
| 12 | Governance AI | AI 辅助 DAO 治理 |
| 13 | Decentralized AI | 模型/数据/算力市场 |
